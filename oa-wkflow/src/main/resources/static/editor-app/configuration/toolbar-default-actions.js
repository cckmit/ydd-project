...
//    		window.location.href = "./";
    ...
    $scope.save = function (successCallback) {
        ...
        // 判断流程定义key是否为空
        if(!json.properties.process_id || json.properties.process_id.length == 0){
        	alert("请输入流程定义key");
        	return;
        }
        // 判断key是否唯一，且存在数字
        var msg = checkModelKey(json.properties.process_id, modelMetaData.modelId);
        if(msg == "FALSE"){
        	alert("流程定义Key已存在，请重新输入");
        	return ;
        }else if(msg == "NUM"){
        	alert("流程定义Key中存在数字，请重新输入");
        	return ;
        }
        
        // 判断流程实例名称是否为空
        if(!json.properties.name || json.properties.name.length == 0){
        	alert("请输入流程实例名称");
        	return;
        }
